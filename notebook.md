...
- ## 行内非替换元素
- 行内框由行高决定
- 没有内边距和边框时，由字体字号和 normal 行高决定
- 设置宽高无效，垂直方向外边距也无效，其他垂直框属性可见但对布局无效
## 行内块元素
- 从外部看其自身，是一个行内元素
- 从内部看元素摆放，内部元素认为自己在块元素内
- 触发自身 BFC （不完全的 BFC）
  - 用最后一行内容的基线作为自身的基线对齐，会影响到外部
- 自动宽度：
  - 最宽不会超出父元素
  - 最窄等于字元素最长的单词（单词不折行）
- 消除空格：父元素设置 `font-size: 0;`
# 7-15
## 定位布局
- 直接指定元素的位置
- `position` 指定定位方式
  - `static` 不定位
  - `fixed` 固定定位，脱离常规流，父元素将其看作不存在
    - 相于视口定位，相当于直接被视口包含
  - `relative` 相对于自身原本位置定位，保留自身原来位置，不脱离常规流，包含块不变
    - 无法通过 `left&right` 来改变大小，因此左右只指定其一，上下亦然
  - `absolute` 绝对定位，脱离常规流
    - 包含块为最近的定位祖先的 `padding-box` ，没有定位祖先时，相对于第一屏定位
  - `sticky` 从视口的某个方向离开时，某方向上的方位属性生效，变为固定定定位
  - `inherit`
- 脱离常规流的定位元素
  - 如果不指定方位，则值为 `auto` ，以原本的起点开始
  - 大小可以被内容撑开
- 元素的覆盖关系：
  - 定位元素盖住常规流元素
  - 常规流内，行内盖住块
  - 定位元素之间，用 `z-index` 指定覆盖顺序，只能为整数
...
- BFC 特性：
  ...
  2. 自动变窄，避开自身以外的浮动元素
    - 不能变窄（例如定宽）时会下移来避开
  3. 不让后代元素 margin 跑到自身外面去
- 清除浮动：向下移动，使两边没有浮动元素
- 闭合浮动：增加自身高度，包裹住浮动元素
  - 定高
  - BFC
  - 在末尾放一个块元素（伪元素）并设置清除浮动，使其下移撑高父元素
